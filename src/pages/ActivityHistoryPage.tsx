    ...
    <div className="space-y-4">
      ...
      <GlassCard>
        <div className="relative">
          <div className="absolute left-[18px] top-2 bottom-2 w-px bg-gradient-to-b from-sky-500/0 via-sky-500/40 to-indigo-500/0 dark:via-sky-400/35" />
          <div className="space-y-4">
            {logs.map((log, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="font-semibold text-slate-900 dark:text-slate-50">{log.productName}</div>
                      <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">
                        {formatDisplayDate(log.createdAt)}
                      </div>
                    </div>
                    <div className="mt-1 text-sm text-slate-700 dark:text-slate-200">{log.message}</div>
                    <div className="mt-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-500">
            ...
            {logs.length === 0 ? <div className="text-sm text-slate-600 dark:text-slate-400">No activity yet.</div> : null}
          </div>
        </div>
      </GlassCard>
    </div>
